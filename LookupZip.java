...
public class LookupZip {
    ...
    public static ExpandableArray<Place> readZipCodes(String filename1, String filename2) throws FileNotFoundException {
        ...
        while (s1.hasNextLine()) {
            ...
            if (line.isEmpty() || line.toLowerCase().contains("zipcode")) continue;
        ...
        while (s2.hasNextLine()) {
            ...
            if (parts.length > 5) {
                try { lat = Double.parseDouble(parts[5].trim()); } catch (NumberFormatException ignored) {}
            }
            if (parts.length > 6) {
                try { lon = Double.parseDouble(parts[6].trim()); } catch (NumberFormatException ignored) {}
            }
            ...
            if (idx >= 0) {
                ...
                if (!Double.isNaN(lat) && !Double.isNaN(lon)) {
                    if (old instanceof PopulatedPlace) {
                        ...
                        places.set(new PopulatedPlace(pp.getZip(), pp.getTown(), pp.getState(), lat, lon, pp.getPopulation()), idx);
                    } else {
                        places.set(new LocatedPlace(old.getZip(), old.getTown(), old.getState(), lat, lon), idx);
                    }
                }
            }
        }
    ...
    // Lookup method using binary search
